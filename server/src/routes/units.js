...
unitsRouter.post('/', requireAuth, async (req, res) => {
  const { type } = req.body ?? {};
  ...
  if (Number(nation.money) < spec.cost) {
  ...
  try {
    ...
    await client.query('UPDATE nations SET money = money - $1 WHERE id = $2', [spec.cost, nation.id]);
...
unitsRouter.post('/:id/move', requireAuth, async (req, res) => {
  ...
  const { rows } = await pool.query(
    `UPDATE units SET dest_lat = $1, dest_lon = $2, updated_at = now()
     ...
     RETURNING id, nation_id AS "nationId", type, lat, lon, dest_lat AS "destLat",
               dest_lon AS "destLon", speed_kmh AS "speedKmh", hp`,
    [lat, lon, req.params.id, nation.id]
  );
